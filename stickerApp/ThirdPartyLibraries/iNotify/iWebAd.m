...
//
//  http://charcoaldesign.co.uk/source/cocoa#iWebAd
//  https://github.com/nicklockwood/iWebAd
//
...
static NSString *const iWebAdIgnoredNotificationsKey = @"iWebAdIgnoredNotifications";
static NSString *const iWebAdViewedNotificationsKey = @"iWebAdViewedNotifications";
static NSString *const iWebAdLastCheckedKey = @"iWebAdLastChecked";
static NSString *const iWebAdLastRemindedKey = @"iWebAdLastReminded";


static iWebAd *sharedInstance = nil;
...
@interface iWebAd() <UIAlertViewDelegate>
#else
@interface iWebAd()
#endif
...
+ (iWebAd *)sharedInstance
{
    ...
    {
        sharedInstance = [[iWebAd alloc] init];
    }
...
- (NSString *)localizedStringForKey:(NSString *)key
{
    ...
    if (bundle == nil)
    {
        ...
        NSString *bundlePath = [[NSBundle mainBundle] pathForResource:@"iWebAd" ofType:@"bundle"];
...
- (id<iWebAdDelegate>)delegate
{
    if (_delegate == nil)
    {
        ...
        _delegate = (id<iWebAdDelegate>)[[UIApplication sharedApplication] delegate];
#else
        _delegate = (id<iWebAdDelegate>)[[NSApplication sharedApplication] delegate];
#endif
...
- (NSDate *)lastChecked
{
    return  [[NSUserDefaults standardUserDefaults] objectForKey:iWebAdLastCheckedKey];
}

- (void)setLastChecked:(NSDate *)date
{
    [[NSUserDefaults standardUserDefaults] setObject:date forKey:iWebAdLastCheckedKey];
    [[NSUserDefaults standardUserDefaults] synchronize];
}

- (NSDate *)lastReminded
{
    return [[NSUserDefaults standardUserDefaults] objectForKey:iWebAdLastRemindedKey];
}

- (void)setLastReminded:(NSDate *)date
{
    [[NSUserDefaults standardUserDefaults] setObject:date forKey:iWebAdLastRemindedKey];
    [[NSUserDefaults standardUserDefaults] synchronize];
}

- (NSArray *)ignoredNotifications
{
    NSArray *ignored = [[NSUserDefaults standardUserDefaults] objectForKey:iWebAdIgnoredNotificationsKey];
...
- (void)setIgnoredNotifications:(NSArray *)keys
{
    ...
    [[NSUserDefaults standardUserDefaults] setObject:keys forKey:iWebAdIgnoredNotificationsKey];
    [[NSUserDefaults standardUserDefaults] synchronize];
}
...
{
    NSArray *viewed = [[NSUserDefaults standardUserDefaults] objectForKey:iWebAdViewedNotificationsKey];
    return viewed ?: [NSArray array];
}

- (void)setViewedNotifications:(NSArray *)keys
{
    [[NSUserDefaults standardUserDefaults] setObject:keys forKey:iWebAdViewedNotificationsKey];
    [[NSUserDefaults standardUserDefaults] synchronize];
}
...
{
    NSLog(@"setNotificationsDic with Notifs :%lu", (unsigned long)[notifications count]);
    ...
    if (notifications != _notificationsDict){
        ...
        for (NSString *key in [filteredNotifications allKeys])
        {
            ...
            NSString *minVersion = [notification objectForKey:iWebAdMessageMinVersionKey];
            NSString *maxVersion = [notification objectForKey:iWebAdMessageMaxVersionKey];
...
- (void)downloadedNotificationsData {
    ...
    NSLog(@"Downloaded Notification Data");
    ...
    {
        if ([self.delegate respondsToSelector:@selector(iWebAdNotificationsCheckDidFailWithError:)])
        {
            [self.delegate iWebAdNotificationsCheckDidFailWithError:self.downloadError];
        }
        
        //deprecated code path
        else if ([self.delegate respondsToSelector:@selector(iWebAdNotificationsCheckDidFailWithError:)])
        {
            NSLog(@"iWebAdNotificationsCheckFailed: delegate method is deprecated, use iWebAdNotificationsCheckDidFailWithError: instead");
            [self.delegate performSelector:@selector(iWebAdNotificationsCheckDidFailWithError:) withObject:self.downloadError];
        }
    ...
    if ([self.delegate respondsToSelector:@selector(iWebAdDidDetectNotifications:)])
    {
        [self.delegate iWebAdDidDetectNotifications:self.notificationsDict];
    }  
    ...
    if (notificationKey)
    {
        ...
        NSString *title = [notification objectForKey:iWebAdTitleKey];
/////
        NSLog(@"Type: %@", [notification objectForKey:iWebAdTypeKey]);
        NSLog(@"Source: %@", [notification objectForKey:iWebAdSourceURLKey]);
        
        
        NSString *message = [notification objectForKey:iWebAdMessageKey];
        NSString *actionURL = [notification objectForKey:iWebAdActionURLKey];
        NSString *actionButtonLabel = [notification objectForKey:iWebAdActionButtonKey] ?: self.defaultActionButtonLabel;
        ...
        //check delegate
        if ([self.delegate respondsToSelector:@selector(iWebAdShouldDisplayNotificationWithKey:details:)])
        {
            if (![self.delegate iWebAdShouldDisplayNotificationWithKey:notificationKey details:notification])
            {
                return;
            }
        }
        ...
        if (!self.visibleAlert)
        {
            ...
            NSLog(@"SHOW SCREEN");
            //NSArray *screens = [[NSBundle mainBundle] loadNibNamed:@"iWebAdView"
              //                                               owner:self options:nil];
            //UIView *view = [screens objectAtIndex:0];
            //view.frame = frame;
            //moduleView2.hidden = YES;
...
- (BOOL)shouldCheckForNotifications
{
    if (!self.debug)
    {
        if (!self.showOnFirstLaunch && [[NSUserDefaults standardUserDefaults] objectForKey:iWebAdIgnoredNotificationsKey] == nil)
        {
    ...
    if ([self.delegate respondsToSelector:@selector(iWebAdShouldCheckForNotifications)])
    {
        return [self.delegate iWebAdShouldCheckForNotifications];
    }
    return YES;
}
...
- (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex
{
    ...
    if (buttonIndex == alertView.cancelButtonIndex)
    {
        ...
        {            
            ...
            //log event
            if ([self.delegate respondsToSelector:@selector(iWebAdUserDidIgnoreNotificationWithKey:details:)])
            {
                [self.delegate iWebAdUserDidIgnoreNotificationWithKey:key details:notification];
            }
        }
        else
        {
    ...
    else if (buttonIndex == 1)
    {
        ...
        //log event
        if ([self.delegate respondsToSelector:@selector(iWebAdUserDidRequestReminderForNotificationWithKey:details:)])
        {
            [self.delegate iWebAdUserDidRequestReminderForNotificationWithKey:key details:notification];
        }
    }
    else
    {
        ...
        //log event
        if ([self.delegate respondsToSelector:@selector(iWebAdUserDidViewActionURLForNotificationWithKey:details:)])
        {
            [self.delegate iWebAdUserDidViewActionURLForNotificationWithKey:key details:notification];
        }
